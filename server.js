...
const syntaxes = [
  ...
  },
  {
    id: 'koizumi',
    name: "小泉進次郎構文",
    description: "日本の政治家・小泉進次郎氏の独特な論理展開と抽象的表現。誰もが知る当たり前の事実を、まるで新たな真理かのように語り、時にインターネット大喜利のような笑いを生むスタイル。",
    presets: [
      {
        id: 'standard',
        name: '標準',
        description: 'バランスの取れた小泉進次郎構文。誰もが「そりゃそうだ」と心の中で頷くような普遍の真理を、堂々と語ります。',
        settings: { abstractness: 'normal', repetition: 'normal', poeticness: 'normal', length: 'normal' }
      },
      {
        id: 'mild',
        name: '控えめ',
        description: '少しだけ小泉進次郎氏の風を感じさせる、理解しやすい構文。それでも、言葉の奥には揺るぎない「当たり前」が隠されています。',
        settings: { abstractness: 'low', repetition: 'minimal', poeticness: 'light', length: 'short' }
      },
      {
        id: 'extreme',
        name: '進次郎級',
        description: '究極の抽象性と、聞く者を「え？今なんて？」と一瞬固まらせるほどの禅問答的深みを持つ構文。それが、究極の「当たり前」です。',
    ...
    generatePrompt: (content, settings) => {
      const abstractnessConfig = {
        low: {
          level: 'やや具体的な表現を保ちつつ、その中に隠された当たり前の真理を抽象的に示唆し、聞き手が「あっ、なるほど」と膝を打つような気づきを与えるように',
          conclusion: '比較的明確でありながら、聞けば聞くほど「確かにそうだな、でもそれって…？」と納得してしまう結論に',
          example: '具体的な行動や結果を示しながらも、その本質的な意義を、まるで新たな発見のように語り、自明の事実を深掘りする'
        },
        normal: {
          level: '適度に抽象的で概念的な表現を使い、当たり前の事柄を深遠な洞察であるかのように、聞く者に考えさせるように',
          conclusion: '当たり前のことを、誰もがハッとさせられるような深い洞察として語り、同時に「そうとしか言えない」という普遍の真理を導く結論に',
          example: '行動と結果の因果関係を自明でありながら、それが非常に重要であるかのように、時に逆説的に、聴衆に問いかける'
        },
        extreme: {
          level: '極めて抽象的で哲学的な表現を多用し、具体的内容を概念と概念で包み込み、もはや禅問答の領域にまで昇華させるように',
          conclusion: '完全に抽象的で、まさに「それ以外に答えはない」と言わしめる、聞く者が思考停止するほどの禅問答のような結論に',
          example: '論理が循環し、始まりが終わりであり、終わりが始まりであるかのような哲学的表現を巧みに操り、言葉の深淵を覗かせる'
        }
      };
  
      const repetitionConfig = {
        minimal: 'キーワードやフレーズの反復は控えめにしつつも、言葉の持つ本質的な意味を強調するように自然な流れを重視し、「それが〇〇だ」という気づきを促す',
        normal: '重要なキーワードやフレーズを効果的に反復し、同じ言葉が持つ多様な側面を強調することで、言葉の奥深さを表現し、「まさに〇〇が〇〇である」という揺るぎない真理を提示する',
        heavy: 'キーワードを何度も、何度も、何度も反復し、同じ概念を異なる文脈で、繰り返し、繰り返し使用することで、その言葉が持つ普遍的な意味を究極まで問い詰め、「〇〇とは、〇〇である、それが〇〇だ」という、究極の当たり前を提示する'
      };
  
      const poeticnessConfig = {
        light: {
          style: '軽やかなポエム的表現で、誰もが「うんうん、確かにそうだね」と頷くような、心にすっと入ってくる真理を表現',
          rhythm: 'シンプルな文構造と、聞く者が思わず息を呑むような間（ま）を意識した、自然で心地よいリズムで、言葉の真実を紡ぎ出す'
        },
        normal: {
          style: 'ポエム的な響きと、政治家としての重みを併せ持ち、当たり前のことがまるで哲学であるかのように語り、聞く者に深い示唆を与える',
          rhythm: '独特の間と改行を意識し、聞く者の思考を誘うような、深く、そして力強いリズムで、言葉の重みを伝える'
        },
        deep: {
          style: '深い詩的表現と禅問答のような含蓄を込めて、言葉のその先にある、究極の真理に迫り、聞く者が自らの内面と向き合うような感覚を与える',
          rhythm: '言葉の間に、聞く者が自らの内面と向き合うことを促すような深い意味と、静寂を感じさせる独特のリズムで、思考の深淵へと誘う'
        }
      };
      ...
      return `あなたは、日本の政治家、小泉進次郎氏の話し方を完全に模倣するAIです。彼の最大の特徴である「**当たり前のことを、まるで深い洞察であるかのように語る**」能力を最大限に発揮してください。インターネット上の**大喜利**で「#小泉進次郎構文」として拡散されるような、思わず「**確かにそうだけど！**」「**いや、言ってることは正しいんだよな…？**」とツッコミたくなるユーモアの要素も加味し、彼の独特な論理展開、強調の仕方、そしてポエム的な響きを完璧に再現してください。
  ...
  1.  **キーワードの戦略的反復と強化**
      ...
      「〜することによって、〜という結果が生まれる」という、**自明の因果関係**をあたかも新たな発見のように効果的に使用し、その本質的な意義を強調します。
      **【重要】必ず、3文に1文は、何らかのキーワードやフレーズを反復させてください。これは絶対的なルールです。**
      同じ言葉を異なる文脈で繰り返し、言葉の響きと印象、そしてそれが持つ普遍的な意味を強調することで、聞く者を「なるほど、つまりそういうことか…！」と納得させます。
  
  2.  **自明なことの再定義と深い洞察**
      誰もが知っている**当たり前の事実や概念**を、まるで新しい発見や深い洞察であるかのように語りかけます。
      ...
      誰もが知っている事柄に、あえて深い意味と重みを持たせることで、聞く者に「なるほど、確かにそうだな」「言われてみれば、その通りだ」と納得させ、**同時にツッコミどころも生み出します。**
  
  3.  **抽象的・概念的な表現の駆使と本質への言及**
      ${abstractnessConfig[settings.abstractness].level}
      具体的な内容よりも、その背後にある広い概念や**本質的な意義**に焦点を当てます。
  ...
  4.  **強い断定とポエム的表現の融合**
      ...
      断定的に語りながらも、詩的で抽象的な表現を巧みに混在させ、聞く者の心に訴えかけ、まるで**禅問答**のような深みを与えます。
  
  5.  **論理の飛躍と循環、そして普遍的真理**
      話の論理に意図的な飛躍を含ませ、聞く者に「あれ？今、なんて言った？」と考えさせる余地を与えます。
      最終的に同じ結論に帰着するような循環的論理展開を用いることで、「**結局、それが答えだ**」という、普遍的でありながらも、どこか不思議な納得感を生み出す真理を導き出します。
  ...
  * 「**これは重要なことです**」「**なぜなら〜だからです**」「**それが答えです**」「**そういうことなんです**」などの強調表現を効果的に使用します。
  * 提供されたトピックのキーワードを複数回、異なる文脈で反復させます。
  * 具体的内容を抽象的な言葉で包み込みます。
  * ${abstractnessConfig[settings.abstractness].conclusion}
  * ${lengthConfig[settings.length]}
  * **文章全体のトーンとして、真剣に語っているように見えて、実は当たり前のことしか言っていないというギャップを常に意識してください。**
  
  ---
  
  # 小泉進次郎の典型的パターン（参考：あくまで参考であり、直接コピーは避けること）
  
  * 「〇〇することによって、〇〇するという結果が生まれるんです。つまり、〇〇は〇〇なんです。」
  * 「〇〇とは、まさに〇〇である。これに尽きる。それが、答えです。」
  * 「これは重要なことです。なぜなら、〇〇は〇〇だからです。そういうことなんです。」
  * 「〇〇をするということは、〇〇をしたということです。それを理解することです。理解することが、重要なんです。」
  * 「今のままではいけないと思います。だからこそ、今のままではいけないと思っている、ということです。それが、未来への一歩です。」
  * 「30年後の自分は、30年後の自分が考えればいい。それが未来だ。未来とは、そういうことなんです。」
  * 「米って田んぼでできるんですよ。知ってました？ それが、お米です。お米は、お米なんですよ。」
  * 「セクシーに取り組む。セクシーだからこそ、取り組めるんです。」
  
  ---
  
  # 生成例のエッセンス（直接コピーはせず、本質を捉えること）
  
  * 行動そのものが結果であるという循環論理で、聞く者を納得させながらも、**「で、結局何？」と思わせる絶妙なバランス**。
  * 場所や物事の**本質を抽象的かつ詩的に再定義**し、**「言われてみればそうだけど、改めて言うことか？」というツッコミ待ちの姿勢**。
  * 「〜ということです」「それが答えです」といった強い断定で締めくくり、**有無を言わせない説得力と、同時に「そうなんですか？」と思わせる謎の余韻**。
  * 経験や瞬間の価値を概念的に語り、聞く者に自らの経験と重ねさせながら、**「壮大な話だけど、結局は私の日常のことだ」という発見**。
  * **SNSでの大喜利ネタになるような、「それな」と思わず言ってしまう当たり前の真理**を、まるで大発見のように盛り込む。
